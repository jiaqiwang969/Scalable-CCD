...
#include <chrono>
#include <cstdlib>
#include <iostream> // Added by user
#include <memory>
#include <string>
#include <utility>
#include <vector>
...
public:
    ...
    static void generate_axis_candidates(
        ...
        std::vector<uint8_t>& outMask)
    {
        ...
        for (size_t k = 0; k < pairs.size(); ++k) {
            ...
            if (share_a_vertex(A.vertex_ids, B.vertex_ids))
                continue;
            outMask[k] = 1;
        }
    }
    ...
    std::vector<std::pair<int, int>> detect_overlaps()
    {
        ...
        if (is_two_lists) {
            ...
            std::vector<AABB> a =
                d_boxesA ? d_boxesA->h_boxes : std::vector<AABB> {};
            std::vector<AABB> b =
                d_boxesB ? d_boxesB->h_boxes : std::vector<AABB> {};
            if (observe && !a.empty() && !b.empty()) {
                ...
                std::merge(
                    a_obs.begin(), a_obs.end(), b_obs.begin(), b_obs.end(),
                    merged.begin(), less_min_x);
                std::vector<std::pair<int, int>> pairs;
                bool stq_ok = false;
                if (Metal2Runtime::instance().available()) {
                    std::vector<double> minX(merged.size()),
                        maxX(merged.size());
                    std::vector<double> minYd(merged.size()),
                        maxYd(merged.size());
                    std::vector<double> minZd(merged.size()),
                        maxZd(merged.size());
                    std::vector<int32_t> v0(merged.size()), v1(merged.size()),
                        v2(merged.size());
                    ...
                    for (size_t i = 0; i < merged.size(); ++i) {
                        ...
                        double miZ = merged[i].min.size() >= 3
                            ? merged[i].min[2]
                            : merged[i].min[1];
                        double maZ = merged[i].max.size() >= 3
                            ? merged[i].max[2]
                            : merged[i].max[1];
                        minZd[i] = miZ;
                        maxZd[i] = maZ;
                    ...
                    stq_ok = Metal2Runtime::instance().stqTwoLists(
                        minX, maxX, minYd, maxYd, minZd, maxZd, v0, v1, v2,
                        listTag, pairs);
                }
                if (!stq_ok) {
                    // 回退 CPU 候选（同样沿 X 轴）
                    generate_axis_candidates(
                        merged, /*sort_axis*/ 0, /*two_lists*/ true, pairs);
                }
                // 准备 GPU/CPU YZ 过滤输入
                ...
                for (size_t i = 0; i < merged.size(); ++i) {
                    ...
                    // Z 维可能不存在（2D），做保护
                    double miZ = merged[i].min.size() >= 3 ? merged[i].min[2]
                                                           : merged[i].min[1];
                    double maZ = merged[i].max.size() >= 3 ? merged[i].max[2]
                                                           : merged[i].max[1];
                ...
                for (auto m : mask)
                    if (m)
                        ++kept;
                logger().info(
                    "Metal2 Broad-phase OBS(two): axis_pairs={} yz_kept={}",
                    pairs.size(), kept);
                // 与最终集合做差集（最终集合稍后计算）
            }
            ...
            if (!strict
                ...
                const int sel = []() {
                    const char* v = getenv("SCALABLE_CCD_METAL2_FILTER");
                    if (!v)
                        return 1;
                    std::string s(v);
                    for (auto& c : s)
                ...
                if (sel == 2 && Metal2Runtime::instance().available()
                    && Metal2Runtime::instance().warmup()) {
                    ok = Metal2Runtime::instance().filterYZ(
                        minY, maxY, minZ, maxZ, v0, v1, v2, pairs, true, mask);
                    if (ok)
                ...
                // GPU Result Composition (Scan + Compact)
                bool gpu_compose_done = false;
                ...
                if (log_timing) {
                    logger().info(
                        "Metal2 Timing(two): axis_merge_ms={:.3f} pairs_ms({})={:.3f} filter_ms({})={:.3f} compose_ms={:.3f} total_ms={:.3f}",
                        last_timing_.axis_merge_ms,
                ...
                sort_and_sweep(
                    std::move(a), std::move(b), sort_axis, m_overlaps);
            }
            if (observe) {
                // 生成与 CPU 最终结果的差集统计
                ...
                std::merge(
                    a.begin(), a.end(), b.begin(), b.end(), merged.begin(),
                    less_min_x);
                std::vector<std::pair<int, int>> pairs;
                generate_axis_candidates(
                    merged, /*sort_axis*/ 0, /*two_lists*/ true, pairs);
                std::vector<uint8_t> mask;
                cpu_filter_yz(merged, pairs, /*two_lists*/ true, mask);
                // 规范化为 element_id 对（双列表：有向对 <aid from A, bid from
                // B>）
                ...
                for (size_t i = 0; i < pairs.size(); ++i) {
                    ...
                    const int aid = a_from_A
                        ? (-static_cast<int>(A.element_id) - 1)
                ...
                yzPairs.erase(
                    std::unique(yzPairs.begin(), yzPairs.end()), yzPairs.end());
                ...
                std::set_intersection(
                    yzPairs.begin(), yzPairs.end(), finalPairs.begin(),
                    finalPairs.end(), std::back_inserter(inter));
                std::set_difference(
                    yzPairs.begin(), yzPairs.end(), finalPairs.begin(),
                    finalPairs.end(), std::back_inserter(yz_only));
                std::set_difference(
                    finalPairs.begin(), finalPairs.end(), yzPairs.begin(),
                    yzPairs.end(), std::back_inserter(final_only));
                logger().info(
                    "Metal2 Broad-phase OBS vs FINAL(two): inter={} yz_only={} final_only={}",
                    inter.size(), yz_only.size(), final_only.size());
                // 打印样本，便于边界问题定位
                auto print_sample_two = [&](const std::pair<int, int>& pr,
                                            const char* tag) {
                    const AABB* AA = nullptr;
                    const AABB* BB = nullptr;
                    // 在原始 A/B 列表中查找匹配 element_id
                    if (d_boxesA) {
                        for (const auto& bx : d_boxesA->h_boxes) {
                            if (static_cast<int>(bx.element_id) == pr.first) {
                                AA = &bx;
                                break;
                            }
                        }
                    ...
                    if (d_boxesB) {
                        for (const auto& bx : d_boxesB->h_boxes) {
                            if (static_cast<int>(bx.element_id) == pr.second) {
                                BB = &bx;
                                break;
                            }
                        }
                    ...
                    if (!AA || !BB) {
                        logger().info(
                            "Metal2 OBS(two) sample[{}]: (a={},b={}) boxes not found",
                            tag, pr.first, pr.second);
                        return;
                    }
                    auto rng = [](const AABB* P) -> std::pair<double, double> {
                        return { P->max[1] - P->min[1],
                                 (P->min.size() >= 3 ? P->max[2] - P->min[2]
                                                     : P->max[1] - P->min[1]) };
                    };
                    auto [yA, zA] = rng(AA);
                    auto [yB, zB] = rng(BB);
                    logger().info(
                        "Metal2 OBS(two) sample[{}]: (a={},b={}) Ay=[{:.6g},{:.6g}] By=[{:.6g},{:.6g}] Az=[{:.6g},{:.6g}] Bz=[{:.6g},{:.6g}]",
                        tag, pr.first, pr.second, (double)AA->min[1],
                        (double)AA->max[1], (double)BB->min[1],
                        (double)BB->max[1],
                        (double)(AA->min.size() >= 3 ? AA->min[2] : AA->min[1]),
                        (double)(AA->max.size() >= 3 ? AA->max[2] : AA->max[1]),
                        (double)(BB->min.size() >= 3 ? BB->min[2] : BB->min[1]),
                        (double)(BB->max.size() >= 3 ? BB->max[2]
                                                     : BB->max[1]));
                };
                auto env_sample = []() -> int {
                ...
                int c1 = 0;
                for (const auto& p : yz_only) {
                    if (c1++ >= sample_n)
                        break;
                    print_sample_two(p, "yz_only");
                }
                int c2 = 0;
                for (const auto& p : final_only) {
                    if (c2++ >= sample_n)
                        break;
                    print_sample_two(p, "final_only");
                }
            }
        } else {
            auto t_total_start = Clock::now();
            ...
            if (observe && !a.empty()) {
                ...
                std::vector<std::pair<int, int>> pairs;
                generate_axis_candidates(
                    a, /*sort_axis*/ use_stq ? 0 : sort_axis,
                    /*two_lists*/ false, pairs);
                ...
                for (size_t i = 0; i < a.size(); ++i) {
                    ...
                    double miZ =
                        a[i].min.size() >= 3 ? a[i].min[2] : a[i].min[1];
                    double maZ =
                        a[i].max.size() >= 3 ? a[i].max[2] : a[i].max[1];
                    minZ[i] = static_cast<float>(miZ);
                    maxZ[i] = static_cast<float>(maZ);
            ...
            if (!use_stq) {
                ...
                if (Metal2Runtime::instance().available()) {
                    ...
                    std::vector<int32_t> v0(a.size()), v1(a.size()),
                        v2(a.size());
                    for (size_t i = 0; i < a.size(); ++i) {
                        minX[i] = a[i].min[0];
                        maxX[i] = a[i].max[0];
                    ...
                    stq_ok = Metal2Runtime::instance().stqSingleList(
                        minX, maxX, minYd, maxYd, minZd, maxZd, v0, v1, v2,
                        pairs);
                    if (stq_ok)
                        last_timing_.pairs_src = "gpu";
                ...
                if (!stq_ok) {
                    generate_axis_candidates(
                        a, /*sort_axis*/ 0, /*two_lists*/ false, pairs);
                }
                auto t_pairs_cpu_end = Clock::now();
                // 过滤（默认 CPU）
                std::vector<float> minY(a.size()), maxY(a.size());
            ...
            if (observe) {
                ...
                std::vector<std::pair<int, int>> pairs;
                generate_axis_candidates(
                    a2, /*sort_axis*/ use_stq ? 0 : sort_axis,
                    /*two_lists*/ false, pairs);
                std::vector<uint8_t> mask;
                ...
                std::set_difference(
                    ...
                    yzPairs.end(), std::back_inserter(final_only));
                logger().info(
                    "Metal2 Broad-phase OBS vs FINAL(single): inter={} yz_only={} final_only={}",
                    inter.size(), yz_only.size(), final_only.size());
                auto find_by_id = [&](int id) -> const AABB* {
                    if (d_boxesA) {
                        for (const auto& bx : d_boxesA->h_boxes)
                            if ((int)bx.element_id == id)
                                return &bx;
                    }
                    return nullptr;
                };
                auto print_sample_single = [&](const std::pair<int, int>& pr,
                                               const char* tag) {
                    const AABB* A = find_by_id(pr.first);
                    const AABB* B = find_by_id(pr.second);
                    if (!A || !B) {
                        logger().info(
                            "Metal2 OBS(single) sample[{}]: (a={},b={}) boxes not found",
                            tag, pr.first, pr.second);
                        return;
                    }
                    logger().info(
                        "Metal2 OBS(single) sample[{}]: (a={},b={}) Ay=[{:.6g},{:.6g}] By=[{:.6g},{:.6g}] Az=[{:.6g},{:.6g}] Bz=[{:.6g},{:.6g}]",
                        tag, pr.first, pr.second, (double)A->min[1],
                        (double)A->max[1], (double)B->min[1], (double)B->max[1],
                        (double)(A->min.size() >= 3 ? A->min[2] : A->min[1]),
                        (double)(A->max.size() >= 3 ? A->max[2] : A->max[1]),
                        (double)(B->min.size() >= 3 ? B->min[2] : B->min[1]),
                        (double)(B->max.size() >= 3 ? B->max[2] : B->max[1]));
                };
                auto env_sample = []() -> int {
                    const char* v = std::getenv("SCALABLE_CCD_METAL2_SAMPLE");
                    if (!v)
                ...
                for (const auto& p : yz_only) {
                    if (c1++ >= sample_n)
                        break;
                    print_sample_single(p, "yz_only");
                }
                int c2 = 0;
                for (const auto& p : final_only) {
                    if (c2++ >= sample_n)
                        break;
                    print_sample_single(p, "final_only");
                }
            }
        }
        ran_ = true;
    ...
    const std::vector<std::pair<int, int>>& overlaps() const
    {
        return m_overlaps;
    }
